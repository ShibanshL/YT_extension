...
function OptionsApp() {
  ...
  const dynamic_Image_Cards = () => {
    ...
    if(tabId.length == 0){
      return(
        <>
          <h1 className="text-red-600 text-left text-[18px] font-bold">Please open a youtube tab for the code to work !</h1>
        </>
      )
    }
    return(
      <>
        <h1 className="text-white text-left text-[18px] font-bold">Please Select a picture.</h1>
        <button className="text-white font-bold text-[15px] rounded-[10px] h-[40px] w-full mt-[-15px] bg-[#00A74D] relative">
          <input
              ...
              />
            <label className=" font-semibold text-[15px] text-white">CHOOSE IMAGE</label>
        </button>
      </>
    )
  }
      ...
      <div className="h-[100vh] w-[100vw] p-[15px] bg-[#0f0f0f] flex items-center justify-center gap-4">
        <div className="h-full w-[20%] flex items-center justify-center flex-col rounded-[10px] gap-4">
          <div className="h-[60%] w-full rounded-[15px] flex items-start justify-between gap-[10px] flex-col bg-[#282828] pt-[40px] pb-[50px] pl-[15px] pr-[15px]">
            ...
            <h2 className="text-white font-bold text-[15px]">YOUR IMAGE :<br></br>
              <span className="font-light text-[18px]">{imageData.name?imageData.name:'~'}</span>
            </h2>
            <h2 className="text-white font-bold text-[15px]">IMAGE SIZE :<br></br>
              <span className="font-light text-[18px]">{imageData.memory?imageData.memory:'~'}</span>
            </h2>
            <h1 className="text-white font-bold">Please provide an image with size lower than 1 MB</h1>
          </div>
          <div className={`h-[40%] w-full rounded-[15px] flex items-start justify-${!image?'between':'between'} flex-col bg-[#282828] p-[15px] `}>
            {dynamic_Image_Cards()}
            {/* <h1 className="text-white text-left text-[18px] font-bold">Do you wanna save or reset ?</h1>
            <div className="w-full h-[60%] flex items-end justify-end flex-col gap-[10px]">
         ...
         <div className="h-[85%] w-full flex items-center justify-center p-[15px]">
            ...
            <div className="h-full w-[80%] flex items-center justify-end gap-3 max-h-full overflow-x-hidden flex-wrap">
              {Array(1, 2, 3, 4, 5, 6, 7 ,8, 9).map((idx:number) => {
                return(
                  <>
                    <div key={idx} className={`flex items-center justify-center flex-col gap-3 h-[140px] w-[220px] ${idx==6?'opacity-0':''}`}>
                      <div className="h-[85%] w-full bg-[#D9D9D9] rounded-[10px] bg-cover bg-center overflow-hidden" style={{backgroundImage: `url(${image})`}}>
                        {/* {image && (
                          <>
                            <img 
